                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <a
                            ...
                            className="text-sm text-gray-600 font-serif hover:text-brand-red transition-colors"
                            aria-label="Report bugs or share feedback"
                        >
                            Feedback
                        </a>
                        <span className="text-gray-300">|</span>
                                            ...
                                            <div className="text-sm text-gray-900 flex flex-wrap items-center gap-2">
                                                ...
                                                {!isLocated && (
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-xs text-gray-500 italic">(unlocated)</span>
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                const search = change.after.trim();
                                                                if (!search) return;
                                                                // Simple client-side find
                                                                // Try to find the text in the rendered output
                                                                // Note: This is hacky because we are searching the DOM, but effective for fallback.
                                                                // We can search for the text content in the scrollable div.
                                                                const container = document.querySelector('.overflow-y-auto');
                                                                if (!container) return;

                                                                // Very basic "find first occurrence"
                                                                // In a real app we might want more complex logic to find the *closest* occurrence
                                                                // matching the context, but this is a fallback.
                                                                ...
                                                                while (node = walker.nextNode()) {
                                                                    if (node.textContent?.includes(search)) {
                                                                        ...
                                                                        container.scrollTo({
                                                                            top: container.scrollTop + (rect.top - containerRect.top) - 100, // Offset
                                                                            behavior: 'smooth'
                                                                        });
                                                            ...
                                                            className="px-1.5 py-0.5 text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 border border-gray-300 rounded cursor-pointer transition-colors"
                                                        >
                                                            Find
                                                        </button>
                                                    </div>
                                                ...
                                                {change.severity !== 'required' && (
                                                    <span className="text-xs text-gray-500">({change.severity})</span>
                                                )}
                                            </div>
                                        </li>
                                    )
